...
RSpec.describe "Build::Abilities", type: :request do
  ...
  context "when logged in" do
    ...
    describe "GET /build/abilities/:id/edit" do
      ...
      # Unlike #index (which still lists the directory server-side and so
      # still refreshes/validates the token up front), #edit no longer makes
      # any GitHub call that would catch an expired refresh token early - it
      # only checks a repo is connected at all, the same as #new always has.
      # The redirect still happens, just one hop later: GithubClient's
      # client-side fetch throws GithubAuthError and redirects itself (see
      # AbilityEditor.test.jsx).
      context "with a connected repository" do
        before { create(:github_installation, user: user, repo_full_name: "nevinera/delve-content") }

        it "renders the JS editor shell with just the key and the (server-only) stock asset catalog - the ability's own content and asset URLs are fetched/resolved client-side, not here" do
          get "/build/abilities/punch/edit"
          ...
          # any asset it references) at all - WebMock would raise if it
          # tried.
          expect(WebMock).not_to have_requested(:get, %r{api\.github\.com/repos/nevinera/delve-content/contents/(abilities|graphics|icons|arc)})
        end
      end
    end
  end
