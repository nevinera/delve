...
RSpec.describe "Build::Registration::Zones", type: :request do
  ...
  context "when not logged in" do
    it "redirects index to login" do
      get "/build/registration/zones"
      expect(response).to redirect_to("/login")
    end

    it "redirects show to login" do
      get "/build/registration/zones/#{zone.id}"
      expect(response).to redirect_to("/login")
    end

    it "redirects new to login" do
      get "/build/registration/zones/new"
      expect(response).to redirect_to("/login")
    end
  end
  ...
  context "when logged in" do
    ...
    describe "GET /build/registration/zones" do
      it "returns 200" do
        get "/build/registration/zones"
    ...
    describe "GET /build/registration/zones/:id" do
      it "returns 200" do
        get "/build/registration/zones/#{zone.id}"
        expect(response).to have_http_status(:ok)
      end
    ...
    describe "GET /build/registration/zones/new" do
      it "returns 200" do
        get "/build/registration/zones/new"
        expect(response).to have_http_status(:ok)
      end
    ...
    describe "POST /build/registration/zones" do
      let(:valid_params) do
      ...
      context "with valid params" do
        it "creates a zone and redirects to show" do
          expect {
            post "/build/registration/zones", params: valid_params
          }.to change(Zone, :count).by(1)
          expect(response).to redirect_to(build_registration_zone_path(Zone.last))
        end
      ...
      context "with an invalid identifier" do
        it "re-renders new with an error" do
          post "/build/registration/zones", params: {zone: valid_params[:zone].merge(identifier: "Bad-Zone")}
      ...
      context "with a duplicate identifier and version" do
        it "re-renders new with an error" do
          post "/build/registration/zones", params: {zone: valid_params[:zone].merge(identifier: "goblin_cave", version: "1.0")}
          expect(response).to have_http_status(:unprocessable_content)
