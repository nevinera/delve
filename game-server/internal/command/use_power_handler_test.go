...
func retryUntilPowerLands(t *testing.T, playerID, targetID uuid.UUID, zone instanceconfig.Zone, payload command.UsePowerPayload, rng *rand.Rand, build func() *instancestate.InstanceState) *instancestate.InstanceState {
	t.Helper()
	for i := 0; i < 200; i++ {
		state := build()
...
func TestUsePowerHandler_MissingUnitIsNoOp(t *testing.T) {
	...
	require.NoError(t, command.UsePowerHandler{Rng: rng}.Handle(uuid.New(), punchPower(), instanceconfig.Zone{}, emptyState()))
}
...
func TestUsePowerHandler_WallElsewhereDoesNotBlock(t *testing.T) {
	...
	retryUntilPowerLands(t, playerID, targetID, zone, punchPower(), rng, func() *instancestate.InstanceState {
...
func TestUsePowerHandler_DamagesTargetInRange(t *testing.T) {
	...
	retryUntilPowerLands(t, playerID, targetID, instanceconfig.Zone{}, punchPower(), rng, func() *instancestate.InstanceState {
		return stateWithPlayerAndTarget(playerID, targetID, 0, 0, 4, 0) // 4ft away, within 5ft range
	})
}
...
func TestUsePowerHandler_HealthDoesNotGoBelowZero(t *testing.T) {
	...
	state := retryUntilPowerLands(t, playerID, targetID, instanceconfig.Zone{}, punchPower(), rng, lowHealthPlayerAndTarget(playerID, targetID))
...
func TestUsePowerHandler_NonFrontalIgnoresFacing(t *testing.T) {
	...
	retryUntilPowerLands(t, playerID, targetID, instanceconfig.Zone{}, payload, rng, func() *instancestate.InstanceState {
...
func TestUsePowerHandler_ClearsTargetOnDeath(t *testing.T) {
	...
	state := retryUntilPowerLands(t, playerID, targetID, instanceconfig.Zone{}, punchPower(), rng, func() *instancestate.InstanceState {
...
func TestUsePowerHandler_SufficientResourceSpendsCost(t *testing.T) {
	...
	state := retryUntilPowerLands(t, playerID, targetID, instanceconfig.Zone{}, costlyPunchPower(), rng, func() *instancestate.InstanceState {
