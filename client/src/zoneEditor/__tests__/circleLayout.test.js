...
describe("circleLayout", () => {
  ...
  it("spaces nodes evenly around a circle sized to fit them all, given an explicit radius/padding", () => {
    const positions = circleLayout(["a", "b", "c", "d"], {nodeRadius: 10, padding: 10});
    const spacing = 10 * 2 + 10; // 30
    const expectedRadius = (spacing * 4) / (2 * Math.PI);
  ...
  it("grows the radius as more nodes are added, keeping per-node spacing constant", () => {
    const few = circleLayout(["a", "b", "c"], {nodeRadius: 10, padding: 10});
    const many = circleLayout(Array.from({length: 12}, (_, i) => `n${i}`), {nodeRadius: 10, padding: 10});
  ...
  it("defaults to NODE_RADIUS for spacing when no override is given", () => {
    ...
    const spacing = NODE_RADIUS * 2 + 30;
    const expectedRadius = (spacing * 2) / (2 * Math.PI);
