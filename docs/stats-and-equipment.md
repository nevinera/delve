...
higher iscore than an item with 10 power (which is 10 focus + 10 piercing), or
...
cost of certain other things have to be specified by the Builder. Items can also
...
score. And the 'consumeable' items (reusable with a cooldown, and limited uses
per private instance) have the same issue.

It's worth noting that all characters will need to accumulate some amount of
many stats to handle more difficult content. A character with very little
